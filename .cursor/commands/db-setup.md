...
データベースのマイグレーションを実行し、シードデータを投入して開発環境をセットアップします。

## 実行前の確認チェックリスト

- [ ] 開発サーバーが停止していることを確認（`local.db` がロックされないように）
- [ ] `package.json` に必要なスクリプトが定義されていることを確認
  - [ ] `db:push` が存在する（開発環境推奨）
  - [ ] `db:generate` が存在する（マイグレーションファイル生成用）
  - [ ] `db:migrate` が存在する（マイグレーション実行用）
  - [ ] `db:seed` が存在する
...
### ステップ2: マイグレーションファイルの生成（オプション）

`db:push`を使用する場合は、このステップは不要です。`db:migrate`を使用する場合のみ必要です。
...
- [ ] マイグレーションファイルを生成
  ...
  ```
- [ ] エラーが発生していないことを確認
- [ ] `drizzle/` ディレクトリに新しいマイグレーションファイルが生成されたことを確認

### ステップ3: マイグレーションの実行

開発環境では、以下の2つの方法があります：

**方法A: `db:push`を使用（推奨・開発環境）**
- [ ] スキーマから直接データベースを更新
  ```bash
  pnpm db:push
  ```
- [ ] マイグレーションファイルを生成せずに直接適用される
- [ ] `local.db` ファイルが新規作成または更新されたことを確認
- [ ] エラーが発生していないことを確認

**方法B: `db:migrate`を使用（本番環境向け）**
- [ ] ステップ2でマイグレーションファイルを生成済みであることを確認
- [ ] マイグレーションを実行
  ```bash
  pnpm db:migrate
  ```
- [ ] マイグレーションが正常に完了したことを確認
- [ ] `local.db` ファイルが新規作成されたことを確認
...
### マイグレーション実行エラー
- `drizzle/` ディレクトリにマイグレーションファイルが存在するか確認
- データベースファイルの権限を確認
- 既存のデータベースファイルがロックされていないか確認
...
**方法B: `db:migrate`を使用（本番環境向け）**
```bash
rm -f local.db local.db-shm local.db-wal && pnpm db:generate && pnpm db:migrate && pnpm db:seed
```
